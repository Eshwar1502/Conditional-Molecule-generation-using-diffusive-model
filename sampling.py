...
from dpm_solvers import get_mol_sampler_dpm3
import time
...
# Change here             
    def mol_pc_sampler(model, n_nodes_pmf,target_properties=None):
# ---------------------------------------------------------------------------------------------------------------------------------#        
# Change ends
...
# ---------------------------------------------------------------------------------------------------------------------------------#                    
# Change here             
            if target_properties is not None:  # NEW: Handle property conditioning
                properties = target_properties.to(device)
            else:
                properties = torch.zeros((atom_shape[0],model.module.num_properties), device=device)
...
# Change here             
                ...
                (x_atom, x_bond), (x_atom_mean, x_bond_mean) = corrector_update_fn((x_atom, x_bond), vec_t, model=model,
                                                                                   atom_mask=atom_mask,
                                                                                   bond_mask=bond_mask,properties= properties)
                x_atom = x_atom * atom_mask.unsqueeze(-1)
                x_bond = x_bond * bond_mask
