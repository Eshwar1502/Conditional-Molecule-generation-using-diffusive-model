...
# NEW CODE 
# ---------------------------------------------------------------------------------------------------------------------------------#             
    def __init__(self, root, name, bond_ch, prop_name= None,
                 conf_dict=None, transform=None, pre_transform=None,
                 pre_filter=None, processed_filename='data.pt'):
...
# ---------------------------------------------------------------------------------------------------------------------------------#
        ...
        if conf_dict is None:
            config_file = pd.read_csv(os.path.join(os.path.dirname(__file__), 'mol_config_updated.csv'), index_col=0)
        ...
        # self.available_prop = str(prop_name) in ast.literal_eval(config['prop_list'])
         ...
         # NEW CODE 
# ---------------------------------------------------------------------------------------------------------------------------------#        
        available_props = ast.literal_eval(config['prop_list'])  # Get all available props

        # Handle property list input
        if prop_name is None:
            self.prop_name = available_props  # Use all properties by default
        else:
            if isinstance(prop_name, str):  # Backward compatibility
                self.prop_name = [prop_name.strip()]
            else:  # Assume list input
                self.prop_name = [str(p).strip() for p in prop_name]

        # Validate ALL requested properties exist
        self.available_prop = all(p in available_props for p in self.prop_name)
...
# ---------------------------------------------------------------------------------------------------------------------------------#        
        if osp.exists(self.processed_paths[0]):
            self.data, self.slices, self.all_smiles ,self.all_props = torch.load(self.processed_paths[0])
...
# ---------------------------------------------------------------------------------------------------------------------------------#
    ...
    def process(self):
        ...
        torch.save((self.data, self.slices, self.all_smiles,self.all_props), self.processed_paths[0])
        print('Done!')
    ...
    def pre_process(self):
        ...
        if self.available_prop:
            prop_list = list(input_df[self.prop_name])
                ...
                # Property handling
        if self.available_prop:
            missing_props = [p for p in self.prop_name if p not in input_df.columns]
            if missing_props:
                raise RuntimeError(f"Missing properties: {missing_props}")
            
            prop_values = input_df[self.prop_name].values.astype(np.float32)


# NEW CODE 
# ---------------------------------------------------------------------------------------------------------------------------------#        
             # Calculate mean/std
                        # Calculate mean and std with numpy first
            prop_values = input_df[self.prop_name].values.astype(np.float32)
            
            # Compute normalization using numpy
            prop_mean = prop_values.mean(axis=0)
            prop_std = prop_values.std(axis=0)
            prop_std[prop_std < 1e-8] = 1.0  # Handle zero std

            stats_path = os.path.join(self.processed_dir, 'prop_stats.pt')
            torch.save({
            'mean': torch.from_numpy(prop_mean),
            'std': torch.from_numpy(prop_std)
                        }, stats_path)

            # Normalize using numpy operations
            normalized_props = (prop_values - prop_mean) / prop_std
            
            
            
            prop_tensor = torch.tensor(normalized_props, dtype=torch.float32)
            self.all_props = prop_tensor
# ---------------------------------------------------------------------------------------------------------------------------------#
            # prop_tensor = torch.tensor(prop_values)            
            # self.all_props = prop_tensor
...
# ---------------------------------------------------------------------------------------------------------------------------------#                    
# Change here             
        if self.name.find('zinc250k_7props') != -1:
            path = os.path.join(self.root, 'raw/valid_idx_zinc250k_7props.json')
            with open(path) as f:
                valid_idx = json.load(f)
# ---------------------------------------------------------------------------------------------------------------------------------#        
# Change ends
...
class ZINC250k_7props(MolDataset):
    """Custom class for 7-property dataset"""
    def __init__(self, root='./', bond_ch=4, prop_name=None, conf_dict=None, transform=None, pre_transform=None, pre_filter=None,
                 processed_filename='data.pt'):
        name = 'zinc250k_7props'  # Must match config name
        super(ZINC250k_7props,self).__init__(root, name, bond_ch, prop_name, conf_dict, transform, pre_transform, pre_filter,
                                    processed_filename)
...
# Change here             
        ...
        elif config.data.name == 'ZINC250k_7props':
            # Handle 7-property specific initialization
            dataset = ZINC250k_7props(
                config.data.root,
                bond_ch=config.data.bond_channels,
                prop_name=config.data.property if hasattr(config.data, 'property') else None,
                transform=transform
            )
...
# Change ends
        ...
        if config.debug_mode:
            # Take first 5000 samples from FULL dataset
            debug_size = 5000
            dataset = dataset[:debug_size]
            all_smiles = dataset.all_smiles[:debug_size]
            all_props = dataset.all_props[:debug_size]

            # Create fresh splits on subset
            total = len(dataset)
            ...
            eval_idx = indices[train_size+test_size:]
        
        else:
            all_smiles = dataset.all_smiles
            all_props = dataset.all_props
