...
def mol_sde_train(config, workdir):
    ...
    for step in range(initial_step, num_train_steps + 1):
        ...
        if step % config.training.log_freq == 0:
            # grad_norm = model.module.prop_encoder[0].weight.grad.abs().mean().item()


            # # Access model through state dictionary
            # model = state['model']
            # if isinstance(model, torch.nn.DataParallel):
            #     grad_norm = model.module.prop_embed[0].weight.grad.abs().mean().item()
            # else:
            #     grad_norm = model.prop_embed[0].weight.grad.abs().mean().item()
                
            # logging.info(f"step: {step}, prop_grad: {grad_norm:.2e}")
            # writer.add_scalar("prop_grad", grad_norm, step)
            ...
            # Check property embedding gradients
            if hasattr(model, 'prop_embed'):
                for idx, layer in enumerate(model.prop_embed):
                    if isinstance(layer, nn.Linear):  # Now uses the imported 'nn'
                        # Check weight gradients
                        if layer.weight.grad is not None:
                            key = f"prop_embed_layer{idx}_weight"
                            grad_debug[key] = layer.weight.grad.abs().mean().item()
                        
                        # Check bias gradients
                        if layer.bias is not None and layer.bias.grad is not None:
                            key = f"prop_embed_layer{idx}_bias"
                            grad_debug[key] = layer.bias.grad.abs().mean().item()
            
            # Log results
            if grad_debug:
                logging.info(f"step: {step}, prop_grads: {grad_debug}")
            else:
                logging.info("No property gradients detected")
...
def mol_sde_evaluate(config, workdir, eval_folder="eval"):
    ...
    for ckpt in range(begin_ckpt, config.eval.end_ckpt + 1):
        ...
        if config.eval.enable_sampling:
            ...
            smile_list = []
            print(f"Total molecules generated: {len(all_samples)}")  
            ...
            if config.eval.save_graph:
                ...
                graph_file = os.path.join(eval_dir, f"{sampler_name}_ckpt_{ckpt}.csv")
                with open(graph_file, "w", newline="") as f:
                    writer = csv.writer(f)
                    writer.writerow(["SMILES"])  # Optional: add a header
