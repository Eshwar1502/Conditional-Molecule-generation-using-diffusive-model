...
class HybridMPBlock(nn.Module):
    ...
    def __init__(self, dim_h,
                 ...
                 temb_dim=None, act=None, dropout=0.0, attn_dropout=0.0,config = None,cross_attn=False):
        super().__init__()
...
# Change here             
        ...
        self.cross_attn = PropertyCrossAttention(
            node_dim=dim_h,
            prop_dim=config.model.property_dim,
            num_heads=num_heads
        ) 

        if self.cross_attn:
            self.norm_cross = nn.LayerNorm(dim_h)
...
# Change ends
    ...
    def forward(self, x, edge_index, dense_edge, dense_index, node_mask, adj_mask, temb=None,pemb=None):
        """
        Args:
...
# Change here             
        ...
        # ========== CROSS-ATTN APPLICATION ==========
        if self.cross_attn and pemb is not None:
            h_2d = h.view(B, N, -1)  # reshape to [B, N, D]
            h_2d = self.cross_attn(h_2d, pemb)
            h = self.norm_cross(h_2d.view(-1, self.dim_h) + h)
        # ========== END CROSS-ATTN ==========
