...
def get_multi_score_fn(atom_sde, bond_sde, model, train=False, continuous=False,properties = None):
    ...
    if isinstance(atom_sde, sde_lib.VPSDE) or isinstance(atom_sde, sde_lib.subVPSDE):
        def score_fn(x, t, *args, **kwargs):
            ...
            if continuous or isinstance(sde, sde_lib.subVPSDE):
                ...
                labels = t * 999
                atom_score, bond_score ,pred_props= model_fn(x, labels, *args, **kwargs)
            ...
            if hasattr(model.module, '_debug_outs'):
                for k, v in model.module._debug_outs.items():
                    writer.add_scalar(f"debug/{k}", v, global_step)
...
def get_multi_theta_fn(atom_sde, bond_sde, model, train=False, continuous=False,*args, **kwargs):
    ...
    if isinstance(atom_sde, sde_lib.VPSDE) or isinstance(atom_sde, sde_lib.subVPSDE):
        def theta_fn(x, t, *args, **kwargs):
            ...
            if continuous or isinstance(sde, sde_lib.subVPSDE):
                ...
                # The maximum value of time embedding is assumed to 999 for continuously-trained models.
                labels = t * 999
                atom_theta, bond_theta , pred_props = model_fn(x, labels, *args, **kwargs)
            else:
                raise NotImplementedError()

            return atom_theta, bond_theta , pred_props
...
def get_theta_fn(sde, model, train=False, continuous=False,*args, **kwargs):
    model_fn = get_model_fn(model, train=train)
