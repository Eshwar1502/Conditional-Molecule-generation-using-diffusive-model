...
from .prop_cross_attn import PropertyCrossAttention
...
class CDGS(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.prop_dim = prop_dim = config.model.property_dim 
...
# Change here             
            ...
            self.prop_embed = nn.Sequential(
                nn.Linear(config.model.num_properties, self.prop_dim),
                nn.LayerNorm(self.prop_dim),
                nn.GELU(),
                nn.Linear(self.prop_dim, self.prop_dim)
            )
            ...
            self.cross_attns = nn.ModuleList([
                PropertyCrossAttention(
                    node_dim=nf,
                    prop_dim=self.prop_dim,
                    num_heads=4
                ) for _ in range(config.model.num_gnn_layers + 1)  # +1 for initial
            ])
...
# Change ends
        ...
        for _ in range(num_gnn_layers):
            modules.append(HybridMPBlock(nf, config.model.graph_layer, "FullTrans_1", config.model.heads,
                                         temb_dim=temb_dim, act=act, dropout=dropout, attn_dropout=dropout,config = config,cross_attn=True))
...
# ---------------------------------------------------------------------------------------------------------------------------------#                    
            # ========== NEW PROPERTY EMBEDDING ==========
        pemb = self.prop_embed(properties)  # <-- ADD THIS
    # ==========

        if self.conditional:
            # Reshape for cross-attention: [B*N, D] -> [B, N, D]
            h_atom_2d = h_atom.view(x_atom.shape[0], -1, self.x_ch)
            # Apply initial cross-attention
            h_atom_2d = self.cross_attns[0](h_atom_2d, pemb)
            # Reshape back: [B, N, D] -> [B*N, D]
            h_atom = h_atom_2d.view(-1, self.x_ch)
...
# Change ends
        ...
        for layer_idx in range(self.num_gnn_layers):
            h_atom, h_dense_edge = modules[m_idx](h_atom, edge_index, h_dense_edge, dense_index,
                                                  atom_mask, adj_mask, temb,pemb)
            m_idx += 1

            # ========== LAYER-WISE CROSS-ATTENTION ==========
            h_atom_2d = h_atom.view(x_atom.shape[0], -1, self.x_ch)
            h_atom_2d = self.cross_attns[layer_idx+1](h_atom_2d, pemb)  # +1 offset!
            h_atom = h_atom_2d.view(-1, self.x_ch)
            # ==========
        ...
        h_graph = h_atom.view(x_atom.shape[0], -1, self.x_ch).mean(dim=1)  # Global pooling
        pred_props = self.prop_predictor(h_graph)

        return atom_score, bond_score , pred_props
