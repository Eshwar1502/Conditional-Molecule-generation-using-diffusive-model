...
# Change here             
        # Code Change
        atom_feat, atom_mask, bond_feat, bond_mask , properties = batch
        # Code change end
# ---------------------------------------------------------------------------------------------------------------------------------#        
# Change ends
...
# Change here             
        # # Code Change
        atom_score, bond_score, pred_props = score_fn(
            (perturbed_atom, perturbed_bond), 
            t, 
            atom_mask=atom_mask, 
            bond_mask=bond_mask,
            properties=properties  # NEW: Pass properties
        )
...
# Change ends
        ...
        property_loss = F.mse_loss(pred_props, properties, reduction='none').mean(dim=1)

        total_loss = (loss_atom + 
                loss_bond + 
                0.1 * property_loss.mean()) 
